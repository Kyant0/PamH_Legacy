    ...
    append {
        ...
        window.fetch(Request("$name/$name.pam.json")).then { response ->
            response.text().then { text ->
                ...
                val images = data.image.associate {
                    "./$name/${it.name.substringBefore("|")}.png" to it.transform
                }
                val sprites = data.sprite.map {
                    images.keys.toList()[it.frame.first().append.first().resource]
                }
                        ...
                        append {
                            div("frame_$name") {
                                ...
                                frames.forEach { (index, change) ->
                                    val image = sprites[resMap.getValue(index)]
                                    val transform = TransformData.parse(images.getValue(image)) then
                                        TransformData.parse(change.transform)
                                    style {
                                        unsafe {
                                            raw(".main_frame_${currentFrame}_$index { ${transform.toCssTransformString()} }")
                                        }
                                    }
                                    img(
                                        src = image,
